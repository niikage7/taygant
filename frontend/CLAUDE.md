...
⚠️ **Что ещё не готово на бэкенде** (возвращает 501): `dashboard`, `workload`, `simulation`, `export`.

Моковых данных в приложении нет. Обзор считается из реальных задач и вех в `src/lib/dashboard-metrics.ts`: прогресс, закрытые вехи, разбивка по статусам, дни до сдачи, задачи с отставанием. Показатели без источника (прогноз отставания, резерв, скорость команды, риски, часы загрузки) **не подставляются выдуманными числами** — на экране стоит заметка о том, каких эндпоинтов ждём.

Компоненты `dashboard/risk-card.tsx`, `dashboard/workload-card.tsx` и `task/cascade-simulation.tsx` готовы, но не подключены — каждый помечен комментарием с эндпоинтом, которого ждёт.
...
Карточка задачи умеет: редактирование (статус, даты, прогресс, описание — PATCH одним запросом по накопленному черновику), чек-лист DoD, вкладки истории и комментариев, создание и удаление связей, переход по связи на другую задачу и копирование ссылки кнопкой «Поделиться».
