...
⚠️ **Что ещё не готово на бэкенде** (возвращает 501): `dashboard`, `workload`, `simulation`, `export`. Поэтому экран `/overview` целиком и блок каскадного сдвига на `/tasks/[taskId]` работают на фикстурах из `src/data/demo.ts` и помечены в интерфейсе плашкой `DemoDataNotice`. Наружу из `demo.ts` торчат только `demoDashboard` и `demoSimulation` — остальное намеренно не экспортируется, чтобы демо-данные не подтянули в экран, у которого уже есть настоящий источник.
